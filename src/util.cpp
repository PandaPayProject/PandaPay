...
const char * const BITCOIN_CONF_FILENAME = "pandapay.conf";
const char * const BITCOIN_PID_FILENAME = "pandapayd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pandapay" is a composite category enabling all PandaPay-related debug output
            if(ptrCategory->count(string("pandapay"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PandaPayCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PandaPayCore
    // Mac: ~/Library/Application Support/PandaPayCore
    // Unix: ~/.pandapaycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PandaPayCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PandaPayCore";
#else
    // Unix
    return pathRet / ".pandapaycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pandapay.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pandapay.conf
