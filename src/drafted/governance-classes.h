 ...
 *
 *   // network
 *   CPandaPayNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.pandapay.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CPandaPayNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CPandaPayNetwork(UniValue objIn)
    {
...
// // can be under: PandaPayNetwork
// //   -- signature requirements : Key1(User)
// class CPandaPayNetworkVariable : public CGovernanceObject
// {
// private:
