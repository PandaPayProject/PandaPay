...
{
    if(fLiteMode) return; // ignore all PandaPay related functionality
...
/*
    ...
    allow endless transaction that would bloat PandaPay and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(out.nValue == 262500*COIN && out.scriptPubKey == payee) return true;
    }
...
{
    if(fLiteMode) return; // disable all PandaPay specific functionality
    ...
    RenameThread("pandapay-privatesend");
