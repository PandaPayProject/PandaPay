...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop PandaPay Core server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "PandaPay Core server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* PandaPay features */
    { "pandapay",               "masternode",             &masternode,             true  },
    { "pandapay",               "masternodelist",         &masternodelist,         true  },
    ...
    { "pandapay",               "getpoolinfo",            &getpoolinfo,            true  },
#ifdef ENABLE_WALLET
    { "pandapay",               "privatesend",            &privatesend,            false },
...
{
    return "> pandapay-cli " + methodname + " " + args + "\n";
}
